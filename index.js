...
function createSymbol() {
  const a = Symbol();
  return a; // Повернемо символ без опису
}
...
function createAndCompareTwoSymbols(value) {
  const symb = Symbol(value);
  const symbo = Symbol(value);
  return symb === symbo;
...
function getSymbolKey(symbol) {
  ...
  return key; // Використовуємо Symbol.keyFor(sym) для отримання ключа, який був використаний для створення символу та повертаємо його
}
...
function useSymbolToStringTag() {
  //   let myObject = {};
  //   const symbol = Symbol("CustomObject");
...
function useSymbolDescription() {
  const mySymbol = Symbol("My Symbol"); // Створюємо символ mySymbol з описом "My Symbol"
  const symbolDescription = mySymbol.description; // Отримуємо опис символу mySymbol за допомогою властивості description
